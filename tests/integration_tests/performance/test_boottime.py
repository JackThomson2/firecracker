...
def test_boottime(
    ...
    for _ in range(10):
        ...
        memory_attr_overhead = events["setting memory attributes"]["duration"]
        metrics.put_metric(
            "build_time",
            build_time.microseconds - memory_attr_overhead,
            unit="Microseconds",
        )
        resume_time = events["boot microvm"]["duration"]
        metrics.put_metric(
            "resume_time",
            resume_time.microseconds - memory_attr_overhead,
            unit="Microseconds",
        )
