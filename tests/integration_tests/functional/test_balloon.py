...
from framework.utils import get_free_mem_ssh, get_stable_rss_mem_by_pid
...
def _test_rss_memory_lower(test_microvm):
    ...
    firecracker_ps = test_microvm.ps
    ...
    init_rss = get_stable_rss_mem_by_pid(firecracker_ps)
...
def test_deflate_on_oom(uvm_plain_any, deflate_on_oom):
    ...
    initial_rss = get_stable_rss_mem_by_pid(firecracker_ps)
...
def test_reinflate_balloon(uvm_plain_any):
    ...
    first_reading = get_stable_rss_mem_by_pid(firecracker_ps)

    # Now inflate the balloon.
    test_microvm.api.balloon.patch(amount_mib=200)
    second_reading = get_stable_rss_mem_by_pid(firecracker_ps)
    ...
    make_guest_dirty_memory(test_microvm.ssh, amount_mib=32)
    third_reading = get_stable_rss_mem_by_pid(firecracker_ps)

    # Now inflate the balloon again.
    test_microvm.api.balloon.patch(amount_mib=200)
    fourth_reading = get_stable_rss_mem_by_pid(firecracker_ps)
...
def test_balloon_snapshot(uvm_plain_any, microvm_factory):
    ...
    fifth_reading = get_stable_rss_mem_by_pid(firecracker_ps)
