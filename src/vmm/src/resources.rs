...
impl VmResources {
    ...
    fn mmds_config(&self) -> Option<MmdsConfig> {
        ...
        let net_devs_with_mmds: Vec<_> = self
            .net_builder
            .iter()
            .filter(|net| net.try_lock().expect("device lock").mmds_ns().is_some())
            .collect();
        ...
        if !net_devs_with_mmds.is_empty() {
            ...
            for net_dev in net_devs_with_mmds {
                let net = net_dev.try_lock().expect("device lock");
                inner_mmds_config
                    .network_interfaces
    ...
    fn set_mmds_network_stack_config(
        ...
        if !network_interfaces.iter().all(|id| {
            self.net_builder
                .iter()
                .any(|device| device.try_lock().expect("device lock").id() == id)
        }) {
        ...
        for net_device in self.net_builder.iter() {
            let mut net_device_lock = net_device.try_lock().expect("device lock");
    ...
    fn allocate_memory_regions(
        ...
        let vhost_user_device_used = self
            ...
            .devices
            .iter()
            .any(|b| b.try_lock().expect("device lock").is_vhost_user());
...
mod tests {
    ...
    fn test_set_vsock_device() {
        ...
        assert_eq!(actual_vsock_cfg.try_lock().unwrap().id(), VSOCK_DEV_ID);
    }
