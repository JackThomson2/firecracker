...
impl Vcpu {
    ...
    /// `ap_ready`, when set, is incremented by the vcpu thread just before
    /// entering its first KVM_RUN.
    pub fn start_threaded(
        ...
        let vcpu_thread = thread::Builder::new()
            ...
            .spawn(move || {
                ...
                barrier.wait();
                self.run(filter, entry_state, ap_ready);
            })
