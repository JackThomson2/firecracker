...
impl VirtioDevice for Block {
    ...
    fn process_rate_limiter_unblock(&mut self) {
        if let Self::Virtio(b) = self {
            ...
            match b.rate_limiter.event_handler() {
                Ok(_) => {
                    let _ = b.process_queue(0);
                }
                Err(err) => {
                    log::error!("block: Failed to handle rate-limiter unblock: {err:?}");
                }
            }
        }
    }
...
impl Persist<'_> for Block {
    ...
    async fn save(&self) -> Self::State {
        match self {
            Self::Virtio(b) => BlockState::Virtio(b.save().await),
            Self::VhostUser(b) => BlockState::VhostUser(b.save().await),
        }
    }
