...
impl Persist<'_> for Balloon {
    ...
    fn restore(
        ...
        let mut balloon = Balloon::new(0, false, state.stats_polling_interval_s, free_page_hinting, free_page_reporting)?;
...
mod tests {
    ...
    fn test_persistence() {
        ...
        assert_eq!(restored_balloon.config_space.num_pages, balloon.config_space.num_pages);
        assert_eq!(restored_balloon.config_space.actual_pages, balloon.config_space.actual_pages);
        assert_eq!(restored_balloon.config_space.free_page_hint_cmd_id, FREE_PAGE_HINT_DONE);
