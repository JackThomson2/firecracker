...
pub struct BlockBuilder {
    ...
    pub devices: VecDeque<Arc<DeviceMutex<Block>>>,
}
...
impl BlockBuilder {
    ...
    pub fn insert(
        ...
        let block_dev = Arc::new(DeviceMutex::new(
...
mod tests {
    ...
    fn test_add_non_root_block_device() {
        ...
        let block = block_devs.devices[0].try_lock().expect("Poisoned lock");
    ...
    fn test_add_root_block_device_first() {
        ...
        assert_eq!(
            block_iter.next().unwrap().try_lock().expect("Poisoned lock").id(),
    ...
    fn test_update() {
        ...
        assert!(block_devs.devices[index].try_lock().expect("Poisoned lock").read_only());
        ...
        assert_eq!(block_devs.devices[0].try_lock().expect("Poisoned lock").id(), root_block_id);
    }
    ...
    fn test_add_device() {
        ...
        block_devs.add_virtio_device(Arc::new(DeviceMutex::new(block)));
        assert_eq!(block_devs.devices.len(), 1);
        assert_eq!(
            block_devs.devices.pop_back().unwrap().try_lock().expect("Poisoned lock").id(),
            block_id
        );
    }
}
