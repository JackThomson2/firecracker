...
mod tests {
    ...
    fn test_add_non_root_block_device() {
        ...
        let block = block_devs.devices[0].lock().expect("Poisoned lock");
    ...
    fn test_add_root_block_device_first() {
        ...
        assert_eq!(
            block_iter.next().unwrap().lock().expect("Poisoned lock").id(),
    ...
    fn test_update() {
        ...
        assert!(block_devs.devices[index].lock().expect("Poisoned lock").read_only());
        ...
        assert_eq!(block_devs.devices[0].lock().expect("Poisoned lock").id(), root_block_id);
    }
    ...
    fn test_add_device() {
        ...
        assert_eq!(block_devs.devices.len(), 1);
        assert_eq!(
            block_devs.devices.pop_back().unwrap().lock().expect("Poisoned lock").id(),
            block_id
        );
    }
}
